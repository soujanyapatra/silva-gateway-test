...
function generate(ts, vueCompilerOptions, sourceTemplate, sourceLang, templateAst, hasScriptSetupSlots, sharedTypesImport, cssScopedClasses = []) {
    const codes = [];
    const formatCodes = [];
    const cssCodes = [];
    const slots = new Map();
    ...
    function createPropsCode(node, props, mode, propsFailedExps) {
        ...
        for (const prop of props) {
            if (prop.type === 7 /* CompilerDOM.NodeTypes.DIRECTIVE */
                && prop.name === 'on'
                && prop.arg?.type === 4 /* CompilerDOM.NodeTypes.SIMPLE_EXPRESSION */) {
                codes.push((0, shared_1.camelize)('on-' + prop.arg.loc.source), ': {} as any, ');
            }
        }
        codes.push(`}, `);
        for (const prop of props) {
